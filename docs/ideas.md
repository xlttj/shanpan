...
## 3. Shadow branch for `knowledge.ndjson`
...
agent made about their code. A shadow branch removes that surface — right
after we concluded that review is the missing piece.

**Therefore the two ideas are coupled:** the shadow branch is defensible *if*
idea 2 (the GUI) provides the replacement review surface. Without it, the
shadow branch removes the only existing gate and puts nothing in its place.

**Mechanics worth getting right:** custom ref (`refs/shanpan/knowledge`)
versus visible orphan branch — invisible and unfetched-by-default versus
teammate-discoverable. Read without checkout via `git show <ref>:<path>`;
write without checkout via `hash-object` / `commit-tree` / `update-ref`, so
nothing ever touches the worktree. Concurrent pushes need a fetch-merge-retry
loop, resolved semantically by `id`. Shallow CI clones and fresh clones do not
get the ref by default — needs an explicit fetch in `init` / `analyze`.
