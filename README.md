...
`knowledge.ref` moves the knowledge log onto a git ref of its own, e.g. `"refs/shanpan/knowledge"`. The file under `.shanpan/` becomes a cache in front of it: reads merge in whatever the ref has gained, and appends commit straight to the ref without ever checking anything out, so recording knowledge cannot dirty the tree you are working in. A ref rather than a branch on purpose — refs outside `refs/heads` are shared across linked worktrees and cannot be checked out, so several worktrees of one repository see one knowledge state and none of them can block another. Run `shanpan upgrade` after setting it to create and seed the ref. Leave it unset (the default) to keep the log in the working tree. `knowledge.commit` decides whether an append also commits to the ref: `auto` (default) or `never`.

`knowledge.remote` (default `origin`), `knowledge.pull` and `knowledge.push` (both `auto` by default, or `never`) govern `shanpan sync`, which fetches the ref, folds both sides together by record id, commits and pushes — one operation, so a machine cannot push what it has without also taking what others wrote. The modes say *whether*, never *when*: the moment is chosen by whichever hook calls sync, and the installed hooks call it on session start and end and after every checkout and merge. There is deliberately no fetch-on-every-read mode; reads happen on nearly every MCP call, and a network round trip on each would make the graph feel broken on a train.

Moving an existing project onto a ref: set `knowledge.ref`, run `shanpan upgrade` to create and seed it, run `shanpan sync` to publish it, then `git rm --cached .shanpan/knowledge.ndjson` and let your `.gitignore` cover `.shanpan/` entirely — the file is cache from that point on. Do the last two steps only once the ref has actually reached the remote, or a fresh clone will find nothing. A clone that has the config but not the ref is not silently empty: record reads and `get_server_info` both say the knowledge has not been fetched and point at `shanpan sync`.
